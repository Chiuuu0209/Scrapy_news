...
# import tutorial.config as config
import config as config
...
def parse(url):
    ...
    if 'money.udn' == source:
        ...
        #     executable_path='./.bin/geckodriver', firefox_options=opt)
        # web.get(url)
    ...
    try:
        date, title, article, keywords = getattr(
            modules['crawler_dev'], source.replace('.', ''))(soup)
...
def blocktempo(soup):
    date = datetime.strptime(
        soup.find(class_='jeg_meta_date').text.strip('\n'), '%Y-%m-%d')
...
def inside(soup):
    date = datetime.strptime(
        soup.find(class_='post_date').text.strip('\n'), '%Y/%m/%d')
    title = soup.find('h1').text
    article = ''
    for paragraph in soup.find('article').find_all('p'):
        article += paragraph.text
    keywords = soup.find(class_='post_meta').find_all(
        'li')[-1].text.replace('\n', '').replace('、', ',')
    return date, title, article, keywords
...
def ithome(soup):
    ...
    title = soup.find('h1').text
    article = soup.find(class_='content-summary').text + \
...
def moneyudn(soup):
    date = datetime.strptime(
        soup.find(class_='shareBar__info').find('span').text, '%Y-%m-%d %H:%M')
    print(date)
...
def technews(soup):
    date = re.sub(r'[\u4E00-\u9FFF\s]', '',
                  soup.select('#main header span.body')[1].text)
    date = datetime.strptime(date, '%Y%m%d%H:%M')
    title = soup.find('h1').text
    article = ''
...
def udncom(soup):
    date = datetime.strptime(
        soup.find(class_='authors').find('time').text, '%Y-%m-%d %H:%M')
    title = soup.find('h1').text
...
def wealthcom(soup):
    date = re.sub(r'[\u4E00-\u9FFF\s\:]', '',
                  soup.select('.entry-header p')[0].text)
...
def wealthhket(soup):
    selects = soup.find(
        class_='article-details-info-container_date').find_all('span')

    date = datetime.strptime(
        selects[0].text + ' ' + selects[1].text, '%H:%M %Y/%m/%d')
    title = soup.find('h1').text
    ...
    keywords = soup.find(
        class_='article-detail_extra-info').find_all('span')[-1].text
    return date, title, article, keywords
...
if '__main__' == __name__:
    ...
    if match:
        ...
        soup = BeautifulSoup(html, 'html.parser')
        date, title, article, keywords = getattr(
            modules['__main__'], source.replace('.', ''))(soup)
        print(date)
        print(title)
        # print(article)
        # print(keywords)
